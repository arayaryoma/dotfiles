---
name: "commit-signed"
description: "Generate commit message and commit on the current (default) branch with GPG signing — never branches off"
---
Execute staging and commit as an explicit, multi-step procedure. Do NOT run
`git add` until step 2 is done. Unlike `commit`, this stays on the current
branch (even `main`/`master`) and GPG-signs every commit.
...
- NEVER stage whole directories. Stage explicit file paths per group:
  `git add <file> <file> ...` then `git commit --gpg-sign`.
- Verify per commit: `git diff --staged --stat` matches the planned file list
  before committing.

## Rules
- Commit on the CURRENT branch. NEVER create a branch, even on `main`/`master`.
- Always GPG-sign: `git commit --gpg-sign` (`-S`).
- Commit messages in English. No emojis.
