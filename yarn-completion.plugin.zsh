...
_yc_get_package_json_property_object() {
  ...
  local property="$2"
  cat "$package_json" |
    gsed -nE "/^ *\"$property\": \{$/,/^ *\},?$/p" | # Grab scripts object
    gsed '1d;$d' |                                   # Remove first/last lines
    gsed -E 's/^\s+"([^"]+)": "(.+)",?/\1=>\2/'      # Parse into key=>value
}
...
_yc_parse_package_json_for_script_suggestions() {
  ...
  _yc_get_package_json_property_object "$package_json" scripts |
    gsed -E 's/(.+)=>(.+)/\1:$ \2/' |  # Parse commands into suggestions
    gsed 's/\(:\)[^$]/\\&/g' |         # Escape ":" in commands
    gsed 's/\(:\)$[^ ]/\\&/g'          # Escape ":$" without a space in commands
}
